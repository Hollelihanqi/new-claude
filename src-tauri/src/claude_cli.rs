...
pub fn resolve_claude_exe() -> Option<PathBuf> {
    let names: &[&str] = if cfg!(target_os = "windows") { NAMES_WINDOWS } else { NAMES_OTHER };
    ...
    let candidates: Vec<PathBuf> = if cfg!(target_os = "windows") {
        ...
        if let Ok(appdata) = std::env::var("APPDATA") { dirs.push(PathBuf::from(appdata).join("npm")); }
        dirs
    } else {
        vec![home.join(".local").join("bin"), home.join(".npm-global").join("bin")]
    };
