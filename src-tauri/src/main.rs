...
fn generate_ps1(list: &[Profile]) -> String {
    ...
    o += "    default { _ccmSync; & $exe @args; _ccmSync -NoWait }\n";
    o += "  }\n}\n";
    o
}
...
// mcp 模块只读实例名集合，不接触 token_enc；保持 load/profile_names/Profile 私有。
...
fn install_integration(list: &[Profile]) -> Result<String, String> {
    ...
    if cfg!(target_os = "windows") {
        ...
        if profile_path.is_empty() {
            return Ok(
                "已生成 cc.ps1，但未能定位 PowerShell $PROFILE，请手动 dot-source 它。".into(),
            );
        }
...
// install_integration 只在增删实例、导入证书或手动同步时调用。升级软件后生成器改了、
...
// 主账户 ~/.claude/settings.json 在 home 目录下启动实例时会以“项目级配置”身份
// 覆盖实例的用户级配置（与 health.rs 对 model 字段的判定同源）。
