    ...
    <div className="view-scroll">
      <Stack gap="md">
        ...
        <SimpleGrid cols={{ base: 1, md: 2 }}>
          ...
          <Card withBorder padding="lg" radius="lg"><Group justify="space-between" mb="md"><Group><ThemeIcon variant="light" size="lg"><IconArchive size={19} /></ThemeIcon><div><Text fw={700}>配置备份</Text><Text size="xs" c="dimmed">导出到桌面；Windows 上这份副本含密钥密文</Text></div></Group></Group><Button variant="light" onClick={backup} loading={backupBusy}>导出到桌面</Button></Card>
        </SimpleGrid>
        <Card withBorder padding="lg" radius="lg"><Group mb="md"><ThemeIcon color="teal" variant="light" size="lg"><IconShieldCheck size={19} /></ThemeIcon><div><Text fw={700}>安全策略</Text><Text size="xs" c="dimmed">当前应用遵循的本地安全边界</Text></div></Group><SimpleGrid cols={{ base: 1, sm: 3 }}><Alert color="teal" icon={<IconLock size={15} />}>网关 Key 加密保存（macOS 钥匙串 / Windows DPAPI）；WorkBuddy 与 MCP 的密钥是明文文件。</Alert><Alert color="blue" icon={<IconShieldCheck size={15} />}>内容安全策略已启用，仅允许本地应用资源和 Tauri 通信。</Alert><Alert color="cyan" icon={<IconArchive size={15} />}>配置使用原子写入，并自动保留最近有效备份。</Alert></SimpleGrid></Card>
      </Stack>
    </div>
  );
}
